...
def xlogyx(x, y):
    ...
    return x * np.log1p((y - x) / x) if x < y else -x * np.log1p((x - y) / y)
...
def wlogyx(w, y, x):
    if x < 1e-100:
        warnings.warn("x is really small return 0")
        return 0.0
    return w * np.log1p((y - x) / x) if x < y else -w * np.log1p((x - y) / y)
...
def compute_bin_lh_f(f, edges, h, w2, extended, use_sumw2, badvalue, *par):
    ...
    return -np.sum(s * (h_pos * np.log(E_pos / h_pos) + (h_pos - E_pos)))
