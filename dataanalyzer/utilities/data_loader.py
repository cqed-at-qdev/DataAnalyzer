...
def load_labber_file(labber_path: str, insepct:bool=False) -> Tuple[Valueclass, ...]:
    def _get_vna_data(step_channels) -> list[Valueclass]:
        ...
        for param in step_channels:
            ...
            if start_freq is not None and stop_freq is not None and n_points is not None and vna_param is not None:
                values = np.linspace(float(start_freq), float(stop_freq), int(n_points))
                return [Valueclass(name=vna_param["name"].replace("Start f", "F"), value=values, unit=vna_param["unit"])]
        return []
    ...
    d = f.getData()
    data = Valueclass(name=log_channel["name"], value=d[0] if len(d) < 2 else d, unit=log_channel["unit"])

    if insepct:
        parameter_names = [param.name for param in parameters] + [data.name]
        print(f"Insepcting Labber File...\nFile is containing {len(parameter_names)} parameters:")
...
def load_json_file(json_path: str, insepct:bool=False) -> Tuple[Valueclass, ...]:
    ...
    parameters = [Valueclass.fromdict(value) for key, value in data_dict["experiment_data"].items()]

    if insepct:
        parameter_names = [param.name for param in parameters]
        print(f"Insepcting Json File...\nFile is containing {len(parameter_names)} parameters:")
        for param in parameter_names:
            print(f"\t{param}")
        print("\n")
