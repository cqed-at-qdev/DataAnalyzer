...
for i in range(n_resonators):
    ...
    resonator_params[f"sigma{number}"] = 0.01 + r.uniform(2, 10)
...
####################################################################################################
# Fit data
func = fitmodels.PolynomialModel(poly_order) + fitmodels.GaussianMultipleModel(
    n_resonators
)
fit = Fitter(x=x_data, y=y_data, func=func)

plot = Plotter()
plot.plot_fit(fit_obejct=fit, plot_metadata=False, plot_residuals=True)
plot.show()
